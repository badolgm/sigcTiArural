...
2. Mover `core/` → `contexts/labs/` primero, por concentrar la mayor parte de la lógica real y ser el contexto más maduro.
   ...
   > **📝 Corrección basada en evidencia (20-jul-2026):** este paso asumía que `core/` (V3) era el origen correcto por ser "el contexto más maduro". Una comparación directa del código confirmó lo contrario: **V3 es una copia funcionalmente idéntica de V2** (`api/logic/domain/`) — hasta conserva strings de artefacto copy-paste sin actualizar (ej. `"AgroNode-Sim (Domain v2)"`, `"Simulado (Domain v2)"`, `"SDR-Simulated (Domain v2)"` dentro de archivos etiquetados como "V3"). Los 50 tests de caracterización de dominio existentes (`tests/domain/`) cubren exclusivamente `api/logic/domain/` (V2); `core/domain/strategies/` (V3) no tiene ningún test. Además, ninguna de las dos capas ejecuta hoy `.procesar()` (la lógica de negocio real) en producción — ambas vistas (`TelemetryHistoryV2View` y `V3View`) solo invocan `.obtener_simulacion_historica()`, y únicamente como fallback cuando no hay lecturas reales en la base de datos — así que no hay riesgo de romper un flujo de negocio activo al elegir una sobre otra. **Conclusión: el origen real de `contexts/labs/` debe ser `api/logic/domain/` (V2), migrando junto con él sus 50 tests.** La única mejora genuina de V3 (el contrato `generar_historico_simulado` como `@abstractmethod` en vez de duck-typing con `hasattr()`) se adopta como mejora aplicada durante la migración, no como razón para partir de V3. El razonamiento original de este paso (arriba) se conserva como referencia histórica del criterio previo a esta verificación — no se borra, se corrige.
